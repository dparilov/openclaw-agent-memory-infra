...
class TestFreshBuild(unittest.TestCase):
    def test_passes(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); code,f=_validate(m); self.assertEqual([x for x in f if x.level=="error"],[]); self.assertEqual(code,0)

class TestMissingMeta(unittest.TestCase):
    def test_exit1(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); (m/"wiki"/"WIKI_META.json").unlink(); code,f=_validate(m); self.assertEqual(code,1); self.assertIn("meta_missing",_codes(f))

class TestMissingSourceFile(unittest.TestCase):
    def test_error(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); (m/"topic-99.md").unlink(); code,f=_validate(m); self.assertEqual(code,1); self.assertIn("source_file_missing",_codes(f))

class TestSha256Mismatch(unittest.TestCase):
    def test_warning(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); (m/"topic-99.md").write_text(SAMPLE+"\n- Extra\n",encoding="utf-8"); _,f=_validate(m); self.assertIn("source_sha256_mismatch",_codes(f))

class TestMtimeNewer(unittest.TestCase):
    def test_warning(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); mp=m/"wiki"/"WIKI_META.json"; meta=json.loads(mp.read_text()); meta["built_at"]="2020-01-01T00:00:00Z"
            mp.write_text(json.dumps(meta,indent=2),encoding="utf-8"); _,f=_validate(m); self.assertIn("source_mtime_newer",_codes(f))
...
class TestLineOutOfRange(unittest.TestCase):
    def test_error(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            mp.write_text(json.dumps(meta,indent=2),encoding="utf-8"); code,f=_validate(m); self.assertIn("line_number_out_of_range",_codes(f)); self.assertEqual(code,1)
...
class TestFactCountMismatch(unittest.TestCase):
    def test_error(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            mp.write_text(json.dumps(meta,indent=2),encoding="utf-8"); code,f=_validate(m); self.assertIn("fact_count_mismatch",_codes(f)); self.assertEqual(code,1)

class TestConflictMismatch(unittest.TestCase):
    def test_error(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); mp=m/"wiki"/"WIKI_META.json"; meta=json.loads(mp.read_text()); meta["conflict_facts"]=999
            mp.write_text(json.dumps(meta,indent=2),encoding="utf-8"); code,f=_validate(m); self.assertIn("conflict_facts_mismatch",_codes(f)); self.assertEqual(code,1)

class TestMissingTopicPage(unittest.TestCase):
    def test_error(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); (m/"wiki"/"topic-99.md").unlink(); code,f=_validate(m); self.assertIn("topic_page_missing",_codes(f)); self.assertEqual(code,1)
...
class TestStrictMode(unittest.TestCase):
    def test_warnings_cause_exit1(self):
        with tempfile.TemporaryDirectory() as t:
            m=_valid(t); mp=m/"wiki"/"WIKI_META.json"; meta=json.loads(mp.read_text()); meta["built_at"]="2020-01-01T00:00:00Z"
            mp.write_text(json.dumps(meta,indent=2),encoding="utf-8"); c1,_=_validate(m,["--strict"]); self.assertEqual(c1,1)
...
class TestGhostTopicNoMismatch(unittest.TestCase):
    def test_no_mismatch(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            meta["source_files"].append({"path":"memory/topic-ghost.md","topic_id":"ghost","fact_count":0,"last_batch":None})
